...
func (r *serviceDiscoveryReconciler) reconcileSupervisorHeadlessService(ctx context.Context, guestClusterCtx *vmwarecontext.GuestClusterContext) error {
	...
	supervisorHosts, err := r.getSupervisorAPIServerAddress(ctx, guestClusterCtx.Cluster)
	if err != nil {
...
func (r *serviceDiscoveryReconciler) getSupervisorAPIServerAddress(ctx context.Context, cluster *clusterv1.Cluster) ([]string, error) {
	...
	if r.NetworkProvider.SupportsIPv6DualStack() {
		supported, err := util.IsDualStackSupported(r.Client)
		if err != nil {
			return nil, err
		}
		dualStackSupported = supported
	}

	// 1. Ensure the v4 single cluster logic doesn't change (or if dual-stack is not supported)
	if !dualStackSupported || ipFamily == util.IPv4SingleStack {
	...
	case util.IPv6SingleStack:
		if ipv6 == "" {
			return nil, errors.Errorf("no IPv6 VIP found for IPv6 single stack cluster")
		}
	...
	case util.DualStackIPv4Primary:
		...
		if len(result) == 0 {
			return nil, errors.Errorf("no VIP found for dual stack cluster")
		}
		return result, nil
	...
	return nil, errors.Errorf("unknown topology")
}
...
func getSupervisorAPIServerVIPs(ctx context.Context, client client.Client) ([]string, error) {
	...
	for _, ingress := range svc.Status.LoadBalancer.Ingress {
		if ipAddr := ingress.IP; ipAddr != "" {
			...
			vips = append(vips, ingress.Hostname)
		}
	}
	if len(vips) > 0 {
		return vips, nil
