...
func updateVMServiceWithVIPs(ctx context.Context, clusterCtx *vmware.ClusterContext, c ctrlclient.Client, cpService CPService, vips ...string) {
	...
	ingresses := make([]vmoprv1alpha5.LoadBalancerIngress, 0, len(vips))
	for _, vip := range vips {
		ingresses = append(ingresses, vmoprv1alpha5.LoadBalancerIngress{IP: vip})
	}
...
var _ = Describe("ControlPlaneEndpoint Tests", func() {
	...
	Context("Reconcile ControlPlaneEndpointService with IPFamily settings (v1alpha6)", func() {
		...
		getVirtualMachineService := func(ctx context.Context, clusterCtx *vmware.ClusterContext, _ ctrlclient.Client, cpService CPService) *vmoprvhub.VirtualMachineService {
			vms, err := cpService.getVMControlPlaneService(ctx, clusterCtx)
			if err != nil {
				if apierrors.IsNotFound(err) || strings.Contains(err.Error(), "owned by a different VSphereCluster instance") ||
					strings.Contains(err.Error(), "exists but is being deleted") {
...
var _ = Describe("VIP helper functions", func() {
	...
	Context("getVMServiceVIP", func() {
		It("should return error if not a LoadBalancer", func() {
			vmService.Spec.Type = vmoprvhub.VirtualMachineServiceTypeClusterIP
			_, err := getVMServiceVIP(vmService)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("does not have load balancer"))
		})

		It("should return error if no ingresses", func() {
			_, err := getVMServiceVIP(vmService)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("does not have any Ingresses"))
		})

		It("should return the first IPv4 VIP", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(vip).To(Equal("10.0.0.1"))
		})

		It("should return the first IPv6 VIP", func() {
			vmService.Status.LoadBalancer.Ingress = []vmoprvhub.LoadBalancerIngress{
				{IP: "fd00::1"},
				{IP: "fd00::2"},
			}
			vip, err := getVMServiceVIP(vmService)
			Expect(err).NotTo(HaveOccurred())
			Expect(vip).To(Equal("fd00::1"))
		})

		It("should return the first VIP regardless of family", func() {
			vmService.Status.LoadBalancer.Ingress = []vmoprvhub.LoadBalancerIngress{
				{IP: "fd00::1"},
				{IP: "10.0.0.1"},
			}
	...
	Context("getVMServiceVIPs", func() {
		It("should return both IPv4 and IPv6 VIPs", func() {
			vmService.Status.LoadBalancer.Ingress = []vmoprvhub.LoadBalancerIngress{
				{IP: "10.0.0.1"},
				{IP: "fd00::1"},
			}
		...
		It("should handle missing families", func() {
			vmService.Status.LoadBalancer.Ingress = []vmoprvhub.LoadBalancerIngress{
				{IP: "10.0.0.1"},
			}
			ipv4, ipv6, err := getVMServiceVIPs(vmService)
...
var _ = Describe("ensureKCPReadyForControlPlaneEndpoint", func() {
	...
	It("should return error when KCP observedGeneration does not match generation", func() {
		...
		err := ensureKCPReadyForControlPlaneEndpoint(context.Background(), c, clusterCtx, []string{ipv4VIP, ipv6VIP})
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("has not yet observed the current generation"))
	})
